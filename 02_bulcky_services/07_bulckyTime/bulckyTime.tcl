...
# Démarrage du RTC
puts  "[clock format [clock seconds] -format "%Y %b %d %H:%M:%S"] : bulckyTime : Démarrage du RTC"
::MCP7940N::start
...
puts  "[clock format [clock seconds] -format "%Y %b %d %H:%M:%S"] : bulckyTime : Vérification de l'heure du RPI"


set piHour [clock seconds]
puts  "[clock format [clock seconds] -format "%Y %b %d %H:%M:%S"] : bulckyTime : L'heure du RPi est [clock format $piHour -format "%Y %b %d %H:%M:%S"]"
...
if {$piHour < 1421942284} {
    ...
    puts  "[clock format [clock seconds] -format "%Y %b %d %H:%M:%S"] : bulckyTime : L'heure du RPi n'est pas bonne"

    # On vérifie maintenant que l'heure du RTC n'est pas déconnante
    if {$rtcHour > 1421942284} {
        puts  "[clock format [clock seconds] -format "%Y %b %d %H:%M:%S"] : bulckyTime : Mise à jour de l'heure du RPi"
        
        #date MMDDhhmmYY.ss
        #date 0319150815.30
        exec sudo date [clock format $rtcHour -format "%m%d%H%M%y.%S"]
        
        puts  "[clock format [clock seconds] -format "%Y %b %d %H:%M:%S"] : bulckyTime : L'heure est mise à jour"

    } else {
        puts  "[clock format [clock seconds] -format "%Y %b %d %H:%M:%S"] : bulckyTime : L'heure du RTC n'est pas bonne"
    }
...
proc updateRTCTime {} {
    ...
    if {$piHour > 1421942284} {
        ...
        puts  "[clock format [clock seconds] -format "%Y %b %d %H:%M:%S"] : bulckyTime : Sauvegarde de l'heure dans le RTC"
        ...
        puts  "[clock format [clock seconds] -format "%Y %b %d %H:%M:%S"] : bulckyTime : L'heure du RPi est incorrecte. On attend une minute avant de sauvegarder l'heure"
